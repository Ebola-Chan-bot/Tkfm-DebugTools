...
const B = 解析('D:/张天夫/TKFM/Tkfm-DebugTools/_精选R12.log');
console.log('R10解析', A.size, '队; R12解析', B.size, '队');
let 同 = 0, 升 = 0, 降 = 0;
const 降队 = [], 升队 = [];
for (const [ids, b] of B) {
  ...
  if (Δ < -0.005) { 降++; 降队.push(`${b.名}[${ids}] R12=${a.终}% → R12=${b.终}% Δ=${Δ}`); }
  else if (Δ > 0.005) { 升++; 升队.push(`${b.名}[${ids}] R12=${a.终}% → R12=${b.终}% Δ=+${Δ}`); }
  else 同++;
}
...
const wA = wallOf('D:/张天夫/TKFM/Tkfm-DebugTools/_精选R10.log'), wB = wallOf('D:/张天夫/TKFM/Tkfm-DebugTools/_精选R12.log');
if (wA && wB) console.log(`串行账总耗时: R12=${wA}s → R12=${wB}s (x${(wA / wB).toFixed(2)})`);
